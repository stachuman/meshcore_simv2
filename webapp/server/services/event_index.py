...
def compute_map_stats(index: EventIndex) -> dict:
    ...
    return {
        ...
        "repeater_stats": index.repeater_stats,
        "time_min": index.time_min,
        "time_max": index.time_max,
    }


def get_messages(index: EventIndex) -> list:
    """Extract sent messages from cmd_reply events, enriched with TX data.

    Standalone version of MapHandler._get_messages — takes an EventIndex
    as parameter instead of reading from self.
    """
    messages = []
    for idx in index.cmd_replies:
        ev = index.events[idx]
        reply = ev.get("reply", "")
        if "msg sent" in reply.lower() or "channel msg sent" in reply.lower():
            ...
            node = ev.get("node", "")
            time_ms = ev.get("time_ms", 0)
            tx = index.find_msg_tx(node, time_ms)
