#!/usr/bin/env bash
# Validate medium-density delay optimization with longer, heavier traffic
# on a DIFFERENT region (Netherlands / Groningen+Friesland) than the sweep
# (Gdansk/Pomerania). This tests whether the best parameters generalize
# to an unseen network topology.
#
# Runs the same topology against (a) default firmware and (b) best optimized
# firmware, each with 6 seeds. Compared to the sweep tests (15min, 4 companions,
# 50 msgs), this uses 1 hour, 16 companions, ~500 direct messages + channels,
# and correlated fading.
#
# Prerequisites:
...
# Best medium parameters from sweep (rank #1)
TX_B=1.5; TX_S=0.6; DTX_B=1.0; DTX_S=0.6; RX_B=0.5; RX_S=0.3
...
lines = [
    ...
    '// Best medium: tx_b=$TX_B tx_s=$TX_S dtx_b=$DTX_B dtx_s=$DTX_S rx_b=$RX_B rx_s=$RX_S',
