// companion_radio/main.cpp
//
// JSON stdin/stdout wrapper for the MeshCore companion_radio MyMesh.
// This compiles the real firmware companion code against Linux shims.
//
// Usage:
//   companion_radio [--name <str>] [--prv <64-byte-hex>] [--sf N] [--bw N] [--cr N]
...
// SimRadio uses aligned storage + placement new (needs clock reference)
...
int main(int argc, char* argv[]) {
    const char* node_name = "companion";
    ...
    // DataStore and serial
    DataStore store(LittleFS, g_clock);
    SimSerial serial_interface;

    // Create the companion MyMesh
    MyMesh mesh(*g_radio_ptr, rng, g_clock, tables, store, nullptr);

    // Load or generate identity
    if (!store.loadMainIdentity(mesh.self_id)) {
        if (prv_hex && strlen(prv_hex) == PRV_KEY_SIZE * 2) {
            uint8_t prv[PRV_KEY_SIZE];
            hex_to_bytes(prv, prv_hex, PRV_KEY_SIZE * 2);
        ...
        store.saveMainIdentity(mesh.self_id);
    }
    ...
    fprintf(stdout,
            "{\"type\":\"ready\",\"pub\":\"%s\",\"role\":\"companion\",\"name\":\"%s\"}\n",
            pub_hex, node_name);
    fflush(stdout);
